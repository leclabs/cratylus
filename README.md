...
| `packages/koine` | the **common tongue** — projection machinery | Universal agent-config translator (formerly _agentir_): author once in a canonical IR, compile to every client dialect (Claude Code, Codex, Cursor, …) and lift any client config back. One Node package (`@leclabs/koine`); `core` · `adapters` · `cli` are source areas under `src/`. Agent memory is the sibling package `packages/episodic`. |
