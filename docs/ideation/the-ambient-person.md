...
_How an ambient agent lives — and how you operate one._

> **Provenance.** This is a human-facing **projection** of the canonical corpus in
> `packages/agent-anatomy/src/` (dimensions · agents · skills · genus). The cells named in `backticks` are the source of truth; this document is a
...
Most "agents" are tools: you prompt, they answer, they forget. An ambient agent is built as a **person**
...
## The anatomy: dimensions, a body, a place

A person is built from a few **dimensions**, each its own canonical idea:
...
- **Memory** (`memory`) — four layers, ordered by how fast they change and who authors
  them: **SOUL** (the fixed essence, generated from the catalog, never hand-edited), **SELF** (the reboot
  ...
  that it once knew).
- **Pulse** (`pulse`) — the clock-dimension: a loop that runs on the person's own cadence — _wake → observe →
...
- **Senses** (`senses`) — the afferent dimension: how it _perceives_. Each sense has a live face (perceive
...
- **Powers** (`powers`) — the efferent dimension: its reach to _act_ (tools, connectors, its voice). The triad
...
**home** (`home`) — a mesh of beings — within a **fleet** (`fleet`), serving an Operator
(`operator-relation`).
...
| You say…              | It drives                                      | In Claude Code today  | Native runtime                 |
| --------------------- | ---------------------------------------------- | --------------------- | ------------------------------ |
...
- **Native runtime** — the robust, purpose-built target: the pulse is a mesh-leased scheduled loop; the senses are a
...
This document was written during a session in which two built-in agents — **Nico** (master builder of the
