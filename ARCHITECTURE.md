...
| **nothing is published yet** — every version is `0.0.0`                                        | **still live.** All six manifests read `version: 0.0.0`. What was cleared on 2026-08-05 was the publish _blocker_, not this divergence: `private: true` retired from `invoke`, `memory`, `runtime`, and all six now declare `publishConfig.access: public`. The `@cratylus` org is owned. Names are free until the first publish and not after                                                                                                                         | **nothing.** No test and no CI step reads a `version` or a `publishConfig`. This row is prose                                                                                                                                                                                      |
...
- **nothing is published yet** — the only unstruck row here — is held by nothing at all.

Two rows held by nothing is the honest count, not a gap to be talked away. This table has already
proved what an unheld row does: the lifecycle-vocabulary row asserted a condition that had been
...
not cover the class. **A property stated only in prose is a property that drifts silently** — the
lesson this section already carried, applied to the section itself.
...
`git grep -lE "from '@cratylus/forge" -- packages/canon/src` names them.
