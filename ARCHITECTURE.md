...
| ~~**canon's own most structural module is still `src/anatomy.ts`**~~ — **REPAIRED 2026-08-05** | the module is `src/manifest.ts`; **177** files reached it (169 `src` + 6 `test`), not the 154 this row claimed. The count was quoted forward and never recomputed                                                                                                                                                                                                                         |
| ~~**a canon cell names the runtime's binary**~~ — **REPAIRED 2026-08-05**                      | the cell now names a FACT, not a value: `workers[].content` carries `{{fact:runtime-bin}}` and the projector substitutes at emission. The byte-anchor was not weakened — its subject moved from the cell's literal to the resolved bytes                                                                                                                                                  |
...
| ~~**property 1 is breached, and a GATE PINS THE BREACH**~~ — **REPAIRED 2026-08-05**           | the pin is gone and `ARCHITECTURE_RATCHET` is **empty**. Property 1 holds with no exceptions. The counter-gate was amended first, as this document required, then the import was deleted                                                                                                                                                                                                  |
| ~~**`FIXTURE_ANATOMY`**~~ — **REPAIRED 2026-08-05**                                            | now `FIXTURE_MANIFEST`. **75** occurrences across 17 files, not the ~110 this row claimed — and it was 75 at the very commit that WROTE ~110, so that figure was never measured at all                                                                                                                                                                                                    |
