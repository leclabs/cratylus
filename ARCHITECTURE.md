...
| divergence                                                                            | evidence                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`schema` imports `runtime`** — an edge this graph does not draw                     | `schema/index.ts` takes `RuntimePlugin` only to derive `keyof Omit<…,'name'>` — it wants a **vocabulary**, not a shape. `shape ⊥ vocabulary`, as `MODEL.md:22` already rules for `Event`. Ratcheted, never licensed |
| **three packages are `private: true`** — including the one owning the installable bin | `invoke`, `memory`, `runtime`; nothing is published and every version is `0.0.0`                                                                                                                                    |
