...
// Seeded-if-absent, NEVER clobbered: `memory init` writes these once when a home
// is provisioned (wake-register), and never overwrites an existing store
...
index (the vault carries the cold corpus). Self-authored, grown at dream; \`memory init\` never
...
write: already-projected ⇒ not stored. Grown at dream (\`correction-consolidation\`); \`memory
init\` never overwrites me. Read whole at wake.*
 ...
 * is the single seed authority: agent-forge no longer carries a parallel copy —
 * it imports THIS export (S6). SOUL (the def) is generated by deploy, not seeded
 * here.
 */
