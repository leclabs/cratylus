...
Each entry: **what it inclines you to do** and **the effect on the agent that holds it**. The _holders_ note records which archetypes carry the heuristic in the corpus.
...
_Holders: principal-ic, developer._
...
_Holders: nico, mav, planner, investigator, boswell._
...
_Holders: principal-engineer-reviewer._
...
_Holders: nico, mav, investigator, boswell._
...
_Holders: nico, mav, arch-doc-writer._
...
No agent holds every heuristic, and few hold only one. An agent's policy shape is the **multi-value set** of heuristics it carries — read off the `holders:` line of each cell. The cells are the source of truth; this section is a reader's view of how they combine.

- **A heuristic is shared, not owned.** Several archetypes can hold the same one (e.g. `emit-at-density` is held by nico, mav, planner, investigator, and boswell). The set is a _selection_, not a partition.
- **The composite is the agent's move-signature.** _nico_ composites `derive-not-transcribe + emit-at-density + use-the-skill + verify-before-assert + whole-not-syntax` — a corpus-builder who re-derives meaning, writes densely, runs the canon rituals, and proves its claims. _mav_ composites `emit-at-density + moonshot-converge-execute + use-the-skill + verify-before-assert` — the same density and rigor, but inclined to reach high and execute autonomously. Same organ, different selection, different mover.
- **To give an agent a heuristic, add its archetype to that cell's `holders:` line** — do not restate the policy on the agent. One heuristic, defined once; many holders. (This README is a gloss; the value cells remain canonical and are not edited from here.)
