...
**Organ:** Reasoning-Strategy (CONATUS — _per-turn · internal_).

Reasoning-Strategy is the **in-context reasoning that weighs options** — planning, chain-of-thought,
search over next moves — taken as _the act of choosing itself_, not as something shown. It is the
mental work an agent does after it has framed the situation (_framing_) and before it commits to
one course (_satisficing_): the turn-by-turn churn of decomposing the problem, generating candidates,
testing them against constraints, and converging on the move worth making. It is the CONATUS-side
twin of _transparency_ — the same reasoning trace, split by whether it is _used_ to choose
(reasoning-strategy) or _presented_ to an audience (transparency).

Each cell in this directory is one canonical reasoning-strategy value: a named _style of weighing_,
authored for the kind of decision its holder actually faces. Unlike a shared-voice organ, there is
**no common genus here** — reasoning-strategy is irreducibly role-specific. A tester weighs absent
...
## How an agent composites its reasoning-strategy

An agent holds **exactly one** reasoning-strategy value, named by its archetype, and that value _is_ the
reasoning method it runs each turn. There is no genus to inherit and nothing to conjoin: reasoning-strategy
does not factor into a shared floor plus role deltas the way _formality_ does, because _how to
...
agents share one. When an agent reaches the reasoning-strategy phase of a turn, it runs the discipline its
single value installs, then closes it at _satisficing_.
