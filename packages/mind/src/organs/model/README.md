...
**Organ:** Model (CONATUS — _persistent · external_) in the agent's conceptual anatomy
(`docs/agent-conceptual-anatomy.md`).

Model is the **inference engine and runtime that actually executes an agent** — the model
...
The model is deliberately an **accident**, not the agent's substance ([[substance-over-accident]]).
...
archetype and its memory-stack, not in the body executing it. The model is _what runs_ the agent;
it is not _what the agent is_. (Compare the STANCE-side twin, _provenance_ — identity as recognized
from outside, what _marks_ the agent — versus model, identity as the machinery that _acts_.)

Each cell in this folder is one canonical model value. The cells are written in the corpus's
...
- **`llm-claude-codeaccident`** — the LLM together with the Claude Code harness, declared as an
  ...
  **Effect on the agent:** it fixes the present runtime (model weights + harness + loop) while
  ...
  a model swap — a new model or harness — and continues as the same individual, with continuity
  ...
  boswell, cognizant): the whole society shares one model today.

## How an agent composites its model

An agent holds **exactly one** model value, named by its archetype. Today every one of the
...
_current_ runtime without binding its identity to it: when the model is swapped, the agent reuses
its memory-stack and persists unchanged. The set is open — were a different runtime introduced, it
would be minted as a new model value and named by the archetypes that run on it.
