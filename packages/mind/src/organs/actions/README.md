...
**Organ:** Actions (APPARATUS — _persistent · external_).

Actions are an agent's **hands** — the standing set of actuators it can invoke to change (or
observe) the world: read a source, call a tool, run code, edit a file, drive a GUI, dispatch a
subagent, send a message, command hardware. Where _modalities_ are the channels by which the world
enters an agent (its eyes), actions are what it reaches out and does. They are _persistent_ (an
agent carries them across every turn, not just one) and _external_ (they act on the world outside
the agent, not on its own reasoning). This is the apparatus side of capabilities: where _capabilities_
is the know-how an agent carries, actions are the concrete instruments through which that know-how
lands as action.
...
## How an agent composites its actions

Unlike organs that grant one value per agent, actions are held as a **set**. An agent binds a
...
actions it holds, and the boundaries between roles are drawn by which hands they were _not_ given.
