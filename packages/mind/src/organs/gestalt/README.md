...
**Industry name:** the agent's _resident situation model_ — the working "gestalt" held in working memory
(also: mental model / global workspace / the construed whole-of-task).

## What gestalt is

An agent's **gestalt** is the **resident hold of the construed semantic whole across a task** — the single
coherent picture of _what the whole job is_ that the agent keeps in mind from the first turn to the last. It
is the standing counterpart to **construal**: where construal is the per-turn act of _framing_ the situation
(forming a fresh interpretation this turn), gestalt is the discipline of _keeping that frame resident_ — never
letting it decay, fragment, or get overwritten by whatever fragment is loudest right now.

The failure gestalt guards against is the most common failure of a long task: **collapsing to the latest
...
verdicts narrow, the elegant design erodes into a pile of local fixes. Gestalt is the organ that refuses this:
it holds the whole resident, and **anchors every step back to that whole** before taking it.

Every value in this organ has the same two-part shape, and that shape _is_ the gestalt discipline:
...
An agent composites gestalt by **including exactly one `hold-*` value** — the one whose _construed whole_ matches its role.
