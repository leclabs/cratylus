...
**Industry name:** _audience-adaptation_ is the **STANCE** organ that the context-engineering anatomy
calls **Audience-Adaptation** — the per-turn, internal modulation of _presentation_ to _this_ interlocutor
and _this_ moment: formality, density, length, mirroring. It is how an agent tunes its face to the
room. It is distinct from **formality** (standing manners that hold across every turn, independent
of audience): audience-adaptation _adapts_ turn-to-turn; formality _persists_.

What audience-adaptation governs is purely how a reply is _dressed_ — not what it claims, not what it
...
This is the default audience-adaptation for most working agents that mainly answer the Operator
turn-by-turn.
...
## How an agent composites audience-adaptation

An agent does not invent its audience-adaptation; it **holds** exactly one of these canonical values by
citing it (`audience-adaptation [[value]]`) in its `agent/<name>.md` selection vector — that vector is the
...
so an agent composites audience-adaptation by selecting the one value whose **reader** and **sharpening
