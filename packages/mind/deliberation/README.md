...
Each value below names its holder and the decision-style it installs.

- **`decomplect-and-locate-pick`** _(principal-ic)_ — decomplect the problem, weigh candidate
...
- **`weigh-options-against-plan`** _(mav)_ — weigh implementation options against the converged
...
- **`decompose-to-method-bearing`** _(planner)_ — decompose toward _method-bearing_ pieces, testing
...
- **`minimal-diff-at-the-hub`** _(developer)_ — weigh the minimal decomplected diff that satisfies
...
- **`decompose-axes-select-oracle`** _(tester)_ — decompose the change into orthogonal axes, and for
...
- **`differential-diagnosis`** _(investigator)_ — run a differential diagnosis: reproduce, separate
...
- **`threat-model-and-rank`** _(principal-engineer-reviewer)_ — threat-model the flow, testing each
...
- **`ontoclean-test-sigma-search`** _(nico)_ — run the OntoClean tests on candidate cuts and search the
...
- **`weigh-representation-verify`** _(arch-doc-writer)_ — weigh how to represent the structure: which
...
- **`partition-and-diff-canon`** _(cognizant)_ — read the lifecycle record, partition each datum
...
- **`weigh-witnessed-vs-reconstructed`** _(boswell)_ — weigh the evidence: separate witnessed fact
