...
Stand up a **project-scope species** — an opinionated, deployable agent — from a universal archetype plus scope accidents. The inverse of [[exemplar-resolution]] applied to an agent cell:
...
4. **Seed the self.** Create the species' home `{home}/{agent}/` and initialize its [[continuity-thread]] (`SELF.md`), so it persists across sessions via the [[memory]] wake protocol rather than waking blank each time.
