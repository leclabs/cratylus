...
The chain's naming ops: `canonical_anchor → coalescence`. Assign each concept its canonical anchor — the reader-relative fittest sign `σ*_R(c)` — then coalesce any concepts that resolve to one anchor (the cut was too fine). This stage `name`s the [[concept-contract]] record: it fills the `anchor` field and preserves the `gloss`, committing no factorization.

Resolve from context: the `produce`d [[concept-contract]] records — each carrying its `gloss`, drawn from the lattice `C_R` (with `prim_R`, `D_R`) — or any set under naming review.

Bindings: `canonical_anchor` makes `α` the reader-relative fittest sign [[signifier-star-r]] (`α(c) = σ*_R(c)`), the densest circumscribing name [[precise-circumscription]], injective and minted-when-none-fits [[anchor-routing]]; `coalesce` is `anchor-routing`'s fuse dual — two concepts under one anchor with no residual distinct load merge [[minimalism]]; `dec_R` binds [[anchor-to-the-readers-priors]] (the priors an anchor fires). The stage fills the `anchor` field of [[concept-contract]] (`gloss` preserved, `factorization` left `⊥`). The symbol table is `references/formal-symbolic-notation.md`.
