...
```text
E ≜ raw items read from ${AGENT_HOME}/EPISODIC.jsonl    -- via memory's `read` verb: `node ~/.claude/skills/memory/episodic.mjs read --home ${AGENT_HOME}` (the store is the source of truth)
