...
- **two-axis routing** ≜ each consolidated item is placed by two orthogonal axes: **type/voice → organ** (identity vs durable-knowledge vs directive vs networked-ref vs next-step), **scope → instance** (which SELF/MEMORY/AGENTS.md/vault). dream adds the two consolidation-only outcomes the resident store has no slot for: `EPISODIC` (kept) and `drop`.
