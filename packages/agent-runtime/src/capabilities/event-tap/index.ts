...
// {@link EventTapHost} observer, reached via `agent-runtime tap <verb>`.
//
// Packaged as a capability MODULE of `@leclabs/agent-runtime` (a subpath export,
// not a standalone `@leclabs/*` package) — see the shard's package-vs-module
// decision. It provides its own Claude harness mapping and depends on NOTHING
// from `@leclabs/agent-forge`: the runtime→forge DAG is never inverted.
//
