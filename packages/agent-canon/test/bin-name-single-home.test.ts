...
//
// The one home is `@leclabs/agent-runtime/bin-name` → `RUNTIME_BIN`. Every consumer
...
//
//   (1) `@leclabs/agent-cli`'s `bin` MANIFEST KEY — the one irreducible second copy
...
const CONSUMERS = [
  'packages/agent-runtime/src/main.ts',
  'packages/agent-cli/src/bin.ts',
  'packages/agent-forge/src/project/runtime-shim.ts',
  'packages/agent-canon/src/hooks/memory-consolidation-nudge.ts',
] as const;
...
describe('the bin name has exactly one home', () => {
  it('no consumer spells the name — only `bin-name.ts` declares it', () => {
    ...
    const home = read('packages/agent-runtime/src/bin-name.ts');
  ...
  it("agent-cli's `bin` manifest key agrees with RUNTIME_BIN", () => {
    ...
    const manifest = JSON.parse(read('packages/agent-cli/package.json')) as {
...
describe('the single-home gate is non-vacuous', () => {
  ...
  it('FAILS on a manifest whose `bin` key drifted from the constant', () => {
    const good = read('packages/agent-cli/package.json');
  ...
  it('FLAGS a consumer that spells the bin name instead of importing it', () => {
    const clean = read('packages/agent-forge/src/project/runtime-shim.ts');
