...
// The bottom arrow, and the only one that writes the substrate. `C ∉ implement` is not
...
export const implement: Role = `implement ≜ reads⟨spec · artifact⟩ → writes⟨artifact⟩
stratum ≜ intent ≺ C ≺ spec ≺ artifact ⟨≺ ≜ more-abstract-than · C ≜ durative-concept-lattice⟩
C ∉ implement ⟨holds spec ¬ design · TWO-descriptions ⇒ validate ≜ operation ¬ re-reading⟩
reserves ⟨author(artifact) ↾ spec · verify ↾ spec-own-criteria⟩
verify ⊥ validate ⟨validate ≜ conformance(C) · C ∉ implement⟩
