// `koine catalog [--corpus <dir>] [--json]` — enumerate an organ-value catalog
// so a builder can DISCOVER the live option-space instead of hard-coding it.
//
// koine owns the mechanism (it types the 24 organs via `ANATOMY`); the corpus
// supplies the data (the value modules under `<corpus>/<organ>/*.ts`). Default
// `--corpus` resolves to mind's `src/organs` when that sibling exists in this
// monorepo — a convenience default, still overridable for ANY organ-module
// corpus (koine stays doctrine-agnostic; mind is just the default data dir).
...
export interface CatalogCmdOpts {
  /** Corpus `organs/` dir; defaults to mind's `src/organs` when present. */
  corpus?: string;
...
/**
 * Resolve the default corpus organs dir: mind's `src/organs`, located relative
...
function defaultCorpus(): string | undefined {
  ...
  // here = <koine>/dist/cli  or  <koine>/src/cli  →  up to <packages>, into mind.
  const candidate = resolve(here, '..', '..', '..', 'mind', 'src', 'organs');
...
export async function runCatalog(opts: CatalogCmdOpts): Promise<number> {
  ...
  if (!corpus) {
    console.error(
      'koine catalog: no --corpus given and no default mind/src/organs found',
    );
    return 1;
  }
  if (!existsSync(corpus)) {
    console.error(`koine catalog: corpus organs dir not found: ${corpus}`);
    return 1;
  }
