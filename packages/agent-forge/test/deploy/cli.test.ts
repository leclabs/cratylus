// CLI-level wiring proof for `koine deploy` (local single-host) + `koine found`
...
describe('parseCompanions', () => {
  it('parses <skill>=<spec> bundle declarations into a companions map', () => {
    const c = parseCompanions('memory=episodic/dist/episodic.mjs', null);
    expect(c).toEqual({ memory: { bundle: ['episodic/dist/episodic.mjs'] } });
  });
...
describe('runDeploy (local single-host)', () => {
  it('deploys agents in-place to <home>/.claude (scope user, --home sandbox)', async () => {
    ...
    process.env[CONFIG_ENV] = join(tmp('polis-empty-'), '.polis.config');
    const { agentsDir, skillsDir } = buildRenderTree(tmp('koine-render-'));
    const home = tmp('koine-home-');
  ...
  it('the bundle hard-error surfaces through the CLI (rc=1) when episodic.mjs is unbuilt', async () => {
    process.env[CONFIG_ENV] = join(tmp('polis-empty-'), '.polis.config');
    const { agentsDir, skillsDir } = buildRenderTree(tmp('koine-render-'));
    const unbuilt = buildBundleSrc(tmp('koine-bundle-'), false); // NOT built
    const home = tmp('koine-home-');
    const rc = await runDeploy({
      agentsDir,
      skillsDir,
