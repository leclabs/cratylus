...
**Subscripts are name-material, not operators**: the Unicode subscript alphanumerics (`₀`–`₉`, `ₐ`–`ₜ`, `ᵢ`–`ᵪ`, `ⱼ`) are definiendum-class — decoration distinguishing locals (`s₁`, `cᵢ`), never table-constrained. An index that **computes** is written in application form (`wave(n+1)`, `W(n)`), never as a subscript expression — the subscript operator glyphs (`₊ ₋ ₌ ₍ ₎`) are deliberately outside the exempt class, so `ₙ₊₁`-style chains fail the gate.
