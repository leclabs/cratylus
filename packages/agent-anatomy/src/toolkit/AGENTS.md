...
`packages/agent-anatomy/src/toolkit/` is agent-anatomy's **build/projection wiring** (the old shell `packages/agent-anatomy/toolkit/`
was consolidated here in T6.3; the Python projector/deployer was retired in T6.1). The typed modules under
`src/` are the **sole source** and **agent-forge** is the only projection + deploy machinery. Here:
...
`skills/memory/` dir carrying the bundled `episodic.mjs`) lives in the agent-forge projector + deploy layer:
...
`place/ssh.py`. `agent-forge deploy --dry-run` enumerates exactly the agents + skills it will place.

**The bin.** `pnpm build` emits the `agent-forge` bin (`@leclabs/agent-forge` `bin.agent-forge` → `dist/cli/index.js`,
shebang'd + executable). When agent-forge is on PATH the operator command is literally `agent-forge deploy …`; the
host-independent invocation (no bin-symlink dependency) is `node packages/agent-forge/dist/cli/index.js
deploy …`, which the convenience scripts use.

**Whole-corpus convenience (run from repo root):**

- `pnpm anatomy:project` — project the TS corpus (`@leclabs/agent-anatomy project`, agent-forge claude adapter) to the
  gitignored render tree `packages/agent-anatomy/.render-ts/` (11 agents + 16 skills incl. the `memory` bundle;
  the projector stages `episodic.mjs` beside `skills/memory/SKILL.md`).
- `pnpm anatomy:deploy` — `build → project → deploy --kind agent → deploy --kind skill → deploy --kind hooks`
...
- `pnpm anatomy:deploy:agent` / `pnpm anatomy:deploy:skill` / `pnpm anatomy:deploy:hooks` — the single-kind halves;
...
**Flags** (`agent-forge deploy --help` for the full set):
...
- `--host <key>` — a `.agent-factory.config` host; omit/`local` deploys in place. `--user <u>` ssh override.
  `--home <dir>` user-scope `.claude` parent (else config home, else `~/.claude`). `--project <dir>`
  for `--scope project`.
- `--fleet` — deploy every `fleet.hosts` minus `fleet.exclude` (needs `.agent-factory.config`). `--exclude
...
- `--dry-run` — print actions, change nothing (the blind-test: `pnpm anatomy:deploy:skill --dry-run`
...
- **Opt-in (per-repo, never checked in).** The flag is `git config --bool agentfactory.continuity`, stored in
...
on every host is **inert** — the worker re-checks the per-repo opt-in flag `git config --bool agentfactory.stanceGuard`
at fire time and exits 0 unless the repo opted in. Still **agent-scoped** (`agentfactory.stanceGuardAgents`, default
