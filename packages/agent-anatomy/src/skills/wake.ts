...
register — \`node ~/.claude/skills/memory/episodic.mjs session register --home \${AGENT_HOME}\` — mark this
          session live in the registry before any orient; liveness (own vs live-other) is the memory-session-isolation axis.
...
orient  — bind to the current project, resume that project's work-thread; read the scoped semantic stores
          ...
          docs). LIVENESS-GATED bind: an \`active/\`-populated plan is bound ONLY if \`owner ∈ {self, completed}\`
          (\`session status <owner>\`); a LIVE OTHER owner ⇒ \`occupied\` — REPORT "plan <P> executed by a live session; not resuming" and fall through to the next candidate. An unfamiliar cwd is a fresh orientation.
