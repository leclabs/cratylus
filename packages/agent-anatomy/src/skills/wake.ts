...
migrate? — host-bootstrap precondition: if \${AGENT_HOME}/EPISODIC.md exists ∧ \${AGENT_HOME}/EPISODIC.jsonl does not → \`node ~/.claude/skills/memory/episodic.mjs migrate \${AGENT_HOME}/EPISODIC.md \${AGENT_HOME}/EPISODIC.jsonl\`; no-loss gated ∧ idempotent (once .jsonl exists it is a no-op)
...
resume — act as the same individual (rebind the continuity-thread); a wake-time read biases the very next action` as SkillExpression;
