...
In the conceptual anatomy (`docs/agent-conceptual-anatomy.md`), **Output-Format** is the _per-turn act_
under the **per-turn · external** cell: after the turn's trigger is taken up, construed, deliberated,
and resolved, output-format is the action _emitted_ — the tool call or token stream actually put into the
world. It is the only organ that leaves the agent and reaches reality; everything upstream is
internal preparation, everything downstream (self-evaluation) is reading the result. Where `capabilities`
says what an agent _can_ do and `satisficing` is the commitment to one course, output-format is the doing
...
agent binds a value by citing `output-format [[value]]` in its `agent/<name>.md` selection vector — the
...
## How an agent composites output-format

An agent does not inline an output format; it _holds_ an output-format form. An agent binds a value by
citing `output-format [[value]]` in its `agent/<name>.md` selection vector — the vector is the single
source of truth. Output-Format is where an agent's role becomes visible as a thing in the world: the form
