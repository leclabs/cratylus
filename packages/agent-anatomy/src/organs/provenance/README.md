...
- A **`principal-ic`-intrinsic** standing — whether the agent carries `principal-ic` as an _intrinsic_ disposition (bound to the polis itself), rather than as a path-scoped grant. This is the difference between an agent that holds `principal-ic` authority intrinsically and a delegate invoked downstream of one.
...
| -------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **principal-ic-root**            | 🏛️·red    | The most-elite-IC root archetype — `principal-ic`-intrinsic-_eligible_. The mission-command root that `mav` and the reviewer specialize. |
| **mav-archetype**                | ✈️·green  | `principal-ic` intrinsic — carries `principal-ic` authority intrinsically.                                                               |
| **nico-archetype**               | 📐·cyan   | `principal-ic` intrinsic — carries `principal-ic` authority intrinsically.                                                               |
| **reviewer-archetype**           | 🛡️·purple | The `principal-ic` archetype specialized to review. `principal-ic` intrinsic.                                                            |
| **tester-archetype**             | ⚖️·purple | `principal-ic` intrinsic — verification lineage.                                                                                         |
...
| **diagnostic-delegate-of-polis** | 🪞·cyan   | A diagnostic _delegate_ of polis — invoked downstream of nico/mav; explicitly **not** `principal-ic`-intrinsic.                          |
...
3. **Inherit its standing.** Whether the lineage is `principal-ic`-intrinsic or a downstream delegate fixes the agent's _recognized authority_ — what it is publicly entitled to do — independent of any task. The `principal-ic`-intrinsic agents (`mav`, `nico`) and their builders carry mission-command authority intrinsically; the `cognizant` delegate explicitly does not.
