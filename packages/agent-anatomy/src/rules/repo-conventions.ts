...
// repo-conventions — the WITNESS `rule` cell (activation=scope): the repo-root
// convention directive. Its canonical DEFINIENS is the σ*-signified identity that
// `accept()` gates; its `body` is the VERBATIM operator-facing directive the
// committed repo-root `AGENTS.md` regenerates from (byte-locked). The claude
// adapter realizes `scope` → a `<dir>/AGENTS.md` the coding agent loads for that
// subtree (here `<dir>` = the repo root). FLAGGED for canon review: the `body` is
// R=human operator prose carried verbatim as the projection payload — the
// canonical-densification of these bodies is the S5/remediation lane, not S4.

export const repoConventions: RuleCell = {
  kind: 'rule',
  id: 'repo-conventions',
  slug: 'repo-conventions',
  definiens:
    'the tree-root directory-scoped directive — the model-as-source-of-truth thesis, the package map, and the commit/build/typecheck invariants every fragment under this scope inherits; a scope-activated rule whose harness target is the root AGENTS.md the coding agent loads.',
  scope: '',
  targetPath: 'AGENTS.md',
  refs: [],
  body: `# agent-factory — agent conventions
