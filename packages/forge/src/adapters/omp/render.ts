...
const TURN_EXEC: readonly string[] = [
  ...
  '    // an outage, and treating it as one blacked out a whole session here.',
  '    if (judgeMisses >= JUDGE_MISS_LIMIT) return undefined;',
  '    try {',
  ...
  '        .join("")',
  '        .trim();',
  '      // An answer clears the run: the latch measures a STANDING outage, never a',
  '      // tally of every slow call a long session accumulates.',
  '      if (text) judgeMisses = 0;',
