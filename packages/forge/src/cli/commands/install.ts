...
export interface InstallCmdOpts {
  ...
  harness?: string;
  /** Corpus package specifier; defaults to whatever mounted this CLI. */
  plugin?: string;
  dryRun?: boolean;
  cwd?: string;
}
...
export async function runInstall(
  ...
  if (harness === undefined) {
    ...
    if (found.length === 0) {
      process.stderr.write(
        `${pc.red('✗')} ${FORGE_BIN} install: no harness found under ${opts.home} — ` +
    ...
    if (found.length > 1) {
      ...
      process.stderr.write(
        `${pc.yellow('!')} ${FORGE_BIN} install: found ${found.join(' and ')} — ` +
  ...
  if (existsSync(configPath)) {
    ...
    if (mod.default === undefined) {
      process.stderr.write(
        `${pc.red('✗')} ${FORGE_BIN} install: '${specifier}' has no default export — a corpus package default-exports its plugin.\n`,
      );
      return 1;
    }
    plugins = [mod.default];
    ...
    process.stderr.write(
      `${pc.red('✗')} ${FORGE_BIN} install: no corpus — write a ${CONFIG_FILE}, pass --plugin <package>, or mount this CLI from a package that names one.\n`,
    );
    return 1;
  }
  ...
  const stage = mkdtempSync(join(tmpdir(), `${FORGE_BIN}-install-`));
  try {
