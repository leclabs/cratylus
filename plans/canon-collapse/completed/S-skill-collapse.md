...
`description` to residue σ\*; collapse the `body` — the skill's **PRIMARY σ\* payload** — to a `formalize`
artifact. Every skill surface (`description` · `body` · `body`) is a **DEPLOYED artifact the model reads
...
- **`body` → a `formalize` artifact** (declarations-above / laws-below, **ZERO explanatory prose**):
...
- **`body`:** its semantic-load content LIFTS into the `body`; residual `body` is held to LLM register
...
- FAIL if a non-empty skill's `body` is dropped, or the `body` carries explanatory prose rather
  than a declarations-above / laws-below `formalize` artifact (E2a REDs).
- FAIL if ANY deployed skill surface (`description` · `body` · `body`) reads as human prose rather than
