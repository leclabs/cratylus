...
every organ value string + every skill `description` + every skill `body` (whole).** Each is read by the
...
A **`body`** (multi-line) is admissible iff it is a `formalize` artifact: every line is a **declaration**
...
- FAIL if the gate's scope omits skill `description` or skill `body` (must cover the whole deployed σ\*
  payload set — organ values + descriptions + bodys — not only organ values).
- FAIL if a `body` with an explanatory-prose line PASSES, or a valid `formalize` artifact
