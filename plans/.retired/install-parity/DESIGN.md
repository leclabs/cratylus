...
|                       | vite                                                      | cratylus                                                                              |
| --------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| build core            | `vite`                                                    | `@leclabs/forge`                                                                      |
| **build-time plugin** | `vite-plugin-*`, listed in `vite.config.ts` `plugins: []` | **`AgentPlugin`**, listed in `agents.config.ts` `extends: []` — e.g. `@leclabs/canon` |
| build invocation      | `vite build`                                              | `<cli> build` / `deploy`                                                              |
| build output          | `dist/` assets                                            | realized harness declarations                                                         |
| runtime core          | vite dev server / preview                                 | `@leclabs/runtime`                                                                    |
| **runtime plugin**    | —                                                         | **`RuntimePlugin`** — `@leclabs/memory`, event-tap                                    |
...
impossible, since N pre-rendered markdown trees cannot be merged. **`canon` is a build-time
plugin and must ship as code.**

Two consequences follow immediately:

- **`canon`'s unpublishability is now BLOCKING, not optional.** It must build to `dist/` with
  ...
  → IR (3 projection)
    → ~/.claude/agents/nico.md (6 realization)      ← harness declaration, REGENERABLE
      → harness reads it, spawns me (7 execution)
        → /wake shim calls `cratylus-run memory …`
...
**The fleet is 7 hosts**, declared in the **gitignored** `.cratylus.config`: `fire` (local),
`forge`, `spark`, `ash`, `apps`, `upmav`, `upgoose`. The committed `.cratylus.config.example`
declares a _different_ 3-host set and has drifted from the live file. `docs/cratylus-config-schema.md`
is cited from three source files and **does not exist**.

- **`upmav` has no checkout of `cratylus`.** It never authors, compiles, projects, or packages.
...
- **Installed at fire's npm prefix**: `@leclabs/runtime` + `@leclabs/memory`, stamp
...
`cratylus`, `packages/…`, `pnpm-workspace`, or any absolute checkout path. Deployed skills invoke
only the `runtime` bin on `PATH`; hook workers reference only `$HOME` paths and `jq`/`git`.
...
1. the CLI is invoked as a _path_ — `node packages/forge/dist/cli/index.js`, never as an
   installed bin;
2. `buildRuntimeBundle` walks up from its own `dist/` location to `pnpm-workspace.yaml` and requires
   `<root>/packages/{runtime,memory}` plus a working `pnpm`;
3. the fleet topology is read from `<nearest .git>/.cratylus.config`;
4. the render tree is `packages/canon/.render-ts`, gitignored and unpublished.

If `forge` were installed _outside_ a workspace, `defaultMonorepoRoot()` would throw and
...
- **`pnpm pack` runs no build.** Neither `runtime` nor `memory` declares `prepack` or
...
- **`canon` is outside the build graph.** It declares no `build` script, so turbo never
...
`packages/runtime/src/loader.ts` discovers capabilities by **ambient resolution**:
...
1. A consumer running `npm i -g @leclabs/runtime` gets **no memory capability**. The install
   succeeds, the bin works, and `cratylus-run memory …` reports the capability absent.
...
A host that deploys **to itself** needs no fleet topology at all. `.cratylus.config` — gitignored,
...
- **`@leclabs/canon` is currently unpublishable** — it exports `./src/index.ts` with
...
- **`runtime` and `memory` are `private: true`.** That flag is the interim state of
...
- `cratylus init` writes an `agents.config.ts` containing `import canon from '@leclabs/canon'`
  - `defineAgentsConfig({ extends: [canon] })`. That is the designed consumer surface.
- **`@leclabs/canon`'s `AgentPlugin` default export has zero live consumers.** There is not one
  real `import … from '@leclabs/canon'` anywhere in the repo — only two string assertions in
...
publish `@leclabs/canon` as raw untranspiled TS with no `dist`, while its transitive runtime
dependency `@leclabs/runtime` is `private: true` and therefore unpublishable — and `forge`
hard-declares `"@leclabs/runtime": "workspace:*"`.
...
embed `` `cratylus-run memory …` `` as **prose inside the formalBlock**, which lands verbatim in the
deployed `SKILL.md` — e.g. `wake/SKILL.md:53-60`, `handoff/SKILL.md:23`. My own `/wake` does this.

`packages/runtime/src/bin.ts:11` states the bin name is a **placeholder** pending S9's rebrand
...
These are already logged in `plans/runtime/PLAN.md:33-36`; this work forces both.
...
- **FORK-3 (publish story) — operator.** Public npm vs private registry vs monorepo-bundled tarball.
  ...
  so this fork can stay open — but the `private: true` flags on `runtime`/`memory` and the
...
- **Projection placement — SETTLED (operator, correcting me).** Projection runs at the **consumer's
  ...
  plugin composition — the defining property of the build-time-plugin architecture. `canon` ships
...
## 7. Distribution is not cratylus's concern
...
`packages/forge` carries **two disjoint pipelines sharing one binary**:
...
`import` and `migrate`. Evidence that B is off the deployment path: **no `.forge/` exists
...
`forge compile claude → 0 files written — the composed set never reaches the IR`, a gap closed by
...
forge, by design: `cratylus deploy` places a render tree into the **local** `.claude/` root, and
