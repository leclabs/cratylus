...
**Objective.** Give `module-scan` its own package subpath export so `canon` stops importing the
...
- `packages/forge/src/core/module-scan.ts` — survives the excision; sits in `core/` but outside
  `core/{ir,engine,serialize,adapter}/`
- `packages/forge/src/core/index.ts` — 11 `export *` lines mixing both lineages
- `packages/forge/package.json` `exports` — note `"."` **and** `"./core"` both resolve to
  `dist/core/index.js`; the package root export _is_ the core barrel
- `packages/canon/src/toolkit/project-cli.ts:27` — imports `resolveModulePath`, `scanCellDirNames`,
  `scanModuleNames` from `@leclabs/forge/core`
...
build; `project-cli.ts` importing through it; zero `@leclabs/forge/core` imports left in
`canon`.

**Completion criteria (falsifier).** `git grep -n "forge/core" -- packages/canon` returns
...
resolves the built artifact; if any other package still reaches `@leclabs/forge/core`; or if the
