...
`packages/canon/src/toolkit/runtime-shim.ts` is a divergent copy of
`packages/forge/src/project/runtime-shim.ts`. The forge original gained
`HARNESS_SESSION_ENV_VARS` — the `CLAUDE_CODE_SESSION_ID → AGENT_SESSION_ID` bridge, commit
`f1621b6`. The canon fork did not. It is live via `packages/canon/src/toolkit/project-cli-codex.ts:26,156`,
...
`packages/canon/src/toolkit/project-cli.ts:31` — belongs to **V7**, which owns that file.
Do not touch `project-cli.ts`; editing a file another wave-0 slice owns is what the disjoint-outputs
law exists to prevent.)

## Inputs

- `packages/canon/src/toolkit/runtime-shim.ts` (to delete)
- `packages/forge/src/project/runtime-shim.ts` (the survivor; note it is NOT exported from the
  package root — check `packages/forge/package.json` `exports` and add a subpath if needed)
- `packages/canon/src/toolkit/project-cli-codex.ts`
...
`packages/canon/src/toolkit/runtime-shim.ts` (deleted) ·
`packages/canon/src/toolkit/project-cli-codex.ts` ·
possibly `packages/forge/package.json` (exports) · a test asserting shim identity across both paths.

## Acceptance

1. `packages/canon/src/toolkit/runtime-shim.ts` does not exist.
