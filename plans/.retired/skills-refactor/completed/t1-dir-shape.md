...
- `packages/agent-canon/src/skills/*.ts` — the 15 cells (`export const <camel>: Skill = {...}`; count 15).
- `packages/agent-canon/src/toolkit/project-cli.ts:57-65` (`moduleNames` glob `*.ts`), `:82` (`skillOf` import), `:116-141` (`projectSkills`, emits `skills/<name>/SKILL.md`); `project-cli-codex.ts:124-139` (identical).
...
- `packages/agent-canon/src/index.ts:26,33` (`dir('./skills')` — likely inert, verify no consumer breaks).
...
REJECTED if: `pnpm --filter @leclabs/agent-canon test` red on any gate; OR `tsc`/`biome` red; OR
`pnpm --filter @leclabs/agent-canon project` produces a `skills/**/SKILL.md` tree that **differs** from
