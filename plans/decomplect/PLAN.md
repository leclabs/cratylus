...
- `deploy-surface` (9) — `t-kind-root-ignores-agent-ext` · `t-init-hardcodes-harness-dir` · `deployed-drifts-from-rendered-unwatched` · `t-signify-marker` · `t-engine-internal-names-await-decode` · `namespaced-pairs-are-a-hand-rolled-map` · `source-can-go-invisible-to-every-text-tool` · `the-drift-worker-infers-three-things-it-should-be-told` · `drift-is-checkable-but-nothing-checks-it`
