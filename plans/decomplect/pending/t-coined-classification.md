# The axiom that says `¬coined` lives inside a dimension classified `coined`

> Census: [`CENSUS-2026-08-05.md`](../CENSUS-2026-08-05.md). **Ruling owed before execution.**

## The contradiction, whole

`schema/src/index.ts:74` — `Classification = 'enum' | 'open' | 'coined'`.
`canon/src/anatomy.ts:64,70,73` — `guardrails`, **`engineering-principles`**, and `heuristics` are
each classified `coined`.

`cratylism.ts:3` — the corpus's **first principle**, stating that a canonical sign is
_INTRINSIC ∧ DISCOVERED ¬coined_ — is a value **of the `engineering-principles` dimension**.

So the axiom forbidding coinage is typed, by the corpus's own manifest, as coined.

## Why it is a real question and not a joke

`coined` is doing genuine work: it marks a dimension whose values are not drawn from a closed
enum **and** are not free-form — the corpus authors them. That is a real third category. The defect
is the SIGN, which asserts exactly what the axiom denies about how the values came to be.

## Why it is blocked

A **mint**. And an unusually load-bearing one: the classification is read by the catalog, the
projector, and `create-agent`'s interview. Getting it wrong renames a concept three tools depend on.

## Constraints

- It does **not** die with `t-project-human-vs-engine` — the sweep recorded them together and they
  are independent. Filed separately for that reason.
- Whatever the derivation returns must survive occupancy against `open`, which is its nearest
  neighbour and may turn out to subsume it.
