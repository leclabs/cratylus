# corpus-hardening — PLAN

Post-founding refinement backlog. Status mirror; task files live under state folders.

## Active

- **cite-once-sweep** · `active/cite-once-sweep.md` · Nico — migrate the older pipeline skills to the
  Bindings-region cite-once convention, each blind-judge-gated; then promote the CITE-TWICE NOTE → FAIL.
  **3/6** — `signify`·`materialize`·`conceptualize` ✓ (`exemplify`·`dream`·`formalize` remaining).

## Frontier (ready)

- **crystallize-verbatim-salience** · `ready/crystallize-verbatim-salience.md` · Nico — make the
  competing-prior ⇒ verbatim-salience rule canon (refine `reader-prior-projection`'s floor).
- **plan-retirement-principle** · `ready/plan-retirement-principle.md` · Nico — crystallize the
  retire-completed-plans policy as a corpus cell.
- **full-fleet-redeploy** · `ready/full-fleet-redeploy.md` · Mav (steward) — propagate the salience fix
  to the other ~5 hosts.
- **prettier-markdown-adoption** · `ready/prettier-markdown-adoption.md` · Mav + Nico — adopt Prettier
  for markdown canonically; verify the 3 corpus preconditions; one-time normalization.

## Backlog (pending)

_(none — the frontier is the backlog; new threads land in `pending/` as they appear.)_

## Done

_(none yet.)_
