# fleet-cutover-v2 — atomic deploy: runtime + seeds + rituals

**Lane** Mav (deploy) + Nico (judge) · **wave(1)** · deps: ⊳runtime-lattice · ⊳forge-seeds ·
⊳corpus-rituals-v2 · HELD.

## Static

All wave(0) completed task-files. **The closed v2 verb surface (pinned here, not implied):**
`encode · read [--under] · node · fold · drain · migrate · audit · lock` — nothing else. Deploy:
`pnpm anatomy:deploy --fleet` (all kinds; no literal `--`; read the log for FLEET-vs-LOCAL blocks).
...
Build → project → fleet deploy → verify → doc reconciliation. No store migration (next wave).

## Accept (falsifiers)

- Per reachable host: sha256(sample ≥3 SOULs) = render; deployed `episodic.mjs` answers exactly the
  pinned verb surface (an off-surface verb answering = fail); a scratch encode on one host derives
