# agent-runtime — SUPERSESSION ledger

> Emitted by `agent-runtime/S2` (plan-reconcile), 2026-07-23. Enumerates how the `agent-runtime`
...
`agent-runtime` decomplects the **BUILD host** (`agent-forge`: project → deploy) from a net-new
**RUNTIME host** (`@leclabs/agent-runtime`: capability port contracts + runtime loader + one branded
host bin). Capability packages (`agent-memory`, `event-tap`) become **runtime plugins** implementing
those ports; a projected skill script becomes a **thin shim → `agent-runtime <capability> <verb>`**.
...
| skills-refactor **T4** compose-build  | ⛔ DIE     | **`agent-runtime/S6`** (+`S8`)              | dep-free-bundle composition retired; S6 reverses projection to thin shims.             |
| skills-refactor **T5** integrate      | ♻ RE-CUT   | **`agent-runtime/S10`**                     | runtime-companion e2e re-cut as integrate-smoke over the runtime install.              |
| event-tap **T1** derive-verbs         | ✅ SURVIVE | → **`agent-runtime/S5`**                    | cold-derived verb set + anchor = naming input to `agent-runtime tap <verb>`.           |
| event-tap **T2** assets-bridge        | ⛔ DIE     | **`agent-runtime/S6`+`S8`** remove the need | thin shim + per-host plugin install ⇒ no per-skill `.sh` asset ships; gap off-path.    |
| event-tap **T3** mechanism            | 🔀 ABSORB  | → **`agent-runtime/S5`**                    | `event-tap.sh` + passive logger become the `EventTapHost` RuntimePlugin impl.          |
| event-tap **T4** skill-cell           | 🔀 ABSORB  | → **`agent-runtime/S8`**                    | `event-tap.ts` becomes a thin-shim skill (drops the `assets:` companion coupling).     |
| event-tap **T5** integrate-deploy     | ♻ RE-CUT   | → **`agent-runtime/S7`+`S10`**              | per-host runtime install (S7) + e2e smoke (S10); clean-worktree + push-reserved carry. |
...
- **skills-refactor E3** (projection-engine + shared-authoring modules out of `agent-canon/toolkit`
  into `agent-forge`) — ORTHOGONAL, out of scope; `agent-runtime/PLAN.md` §Relation confirms.
- The **c13e911 home-resolution** work (`requireHome`/`memory home`/seed-target→`~/.agents`) is
  preserved, folded into `agent-runtime/S4` (MemoryStrategy) + `S7` (install subsumes seed-target) —
