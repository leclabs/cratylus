...
Change class: **feature (neutral output surface + importer)**. Territory: new `src/adapters/standards/**` (canonical id from the E7.S9 probe set: `agentsmd¦agents-md¦standards¦agnostic¦neutral` — implementer picks one, test accepts any probed name) · `src/cli/commands/doctor.ts` (mirror-drift check) · engine emission glue in `src/core/engine/**` limited to nested-AGENTS.md/.agents-skills routing (no adapter-dialect edits — w5 adapter dirs are foreign territory).
...
- Territory: production diff confined to declared territory + roster seam; graduation flips are the only test edits.
